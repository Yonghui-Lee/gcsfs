...
def _read_op_rand(gcs, file_paths, chunk_size, offsets, runtime, block_size, mrd_pool_size=None):
...
def _read_op_repeatedly_open_same_file(gcs, file_paths, chunk_size, runtime, block_size, mrd_pool_size=None):
...
def _random_read_worker(gcs, file_paths, chunk_size, offsets, runtime, block_size, mrd_pool_size=None):
...
def _build_read_worker(params, gcs, file_paths):
    ...
    op_args = (gcs, file_paths, params.chunk_size_bytes, params.runtime, block_size, mrd_pool_size)
...
def _process_worker_fixed_duration(
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        if pattern == "seq":
            futures = [
                executor.submit(
                    _read_op_seq, gcs, file_paths, chunk_size, runtime, block_size, mrd_pool_size
                )
            ...
            futures = [
                executor.submit(
                    _read_op_repeatedly_open_same_file, gcs, file_paths, chunk_size, runtime, block_size, mrd_pool_size
                )
                for _ in range(threads)
            ]
        elif pattern == "rand":
