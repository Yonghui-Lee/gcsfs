...
def _coalesce_generation(*args):
    ...
    # Optimization: Avoid set instantiation for small parameter lists in hot paths.
    val = None
    for arg in args:
        if arg is not None:
            if val is not None and val != arg:
                # Construct the set only within the error handling block to preserve
                # original string formatting without performance penalties.
